...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const rsvp = await prisma.meetingRsvp.upsert({
    where: { meetingId_memberAccountId: { meetingId: params.id, memberAccountId: account.id } },
    create: { meetingId: params.id, memberAccountId: account.id, status, guestCount: guestCount ?? 1, note: note || null },
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  await prisma.meetingRsvp.deleteMany({ where: { meetingId: params.id, memberAccountId: account.id } });
