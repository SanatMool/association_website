    ...
    <div className="min-h-screen bg-slate-50 pt-24">
      <div className="bg-navy-900 py-16 px-4">
        <div className="max-w-7xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            ...
            <h1 className="font-serif text-4xl sm:text-5xl font-bold text-white mb-3">{t.news.title}</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="flex flex-wrap gap-3 mb-8">
          {categories.map((cat) => (
            <button key={cat} onClick={() => setFilter(cat)} className={`text-sm font-medium px-4 py-2 rounded-full capitalize transition-all ${filter === cat ? "bg-navy-900 text-gold-400" : "bg-white border border-slate-200 text-slate-600 hover:border-slate-300"}`}>
