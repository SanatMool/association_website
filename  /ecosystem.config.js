	...
	apps: [
		{
			...
			/*** Script to run. * We run Node explicitly so we can control memory usage. */
			script: "node",

			/*** Arguments passed to the script. * This starts the Next.js production server on port 3002
			 * while limiting Node.js memory usage to prevent spikes on low-RAM VPS.
			 */
			args: "--max-old-space-size=180 node_modules/.bin/next start -p 3002",
