...
## Feature — Per-Association Homepage Content (Hero, About, Mission, Why Join) + Asset Upload Fix ✓ COMPLETE (2026-08-30)

Reviewing a live person-mode association surfaced that almost none of the homepage was actually per-association: Hero background images and About's photo were hardcoded stock Unsplash URLs shared by every association in a mode, Mission/WhyJoin's 6 cards each were global i18n copy with zero admin control, the favicon/default-member-image settings were paste-a-URL-only with no real upload, and the admin dashboard checklist still said "Add member venues" in person mode.

- **Schema**: `Association.homepageContent Json?` (migration `20260830085806_add_homepage_content_to_association`) — one additive field holding `{ heroSlides?, aboutImage?, aboutHeadline?, aboutBadge?, missionItems?, whyjoinItems? }` (shape in `lib/homepage-content.ts`), instead of a separate column per field, so future additions to this bundle don't need another migration.
- **Icon registry**: `lib/homepage-content.ts`'s `SECTION_ICONS` — a curated ~24-icon lucide set (same "bounded choices, not arbitrary" principle as the color presets) used both by the public Mission/WhyJoin components (name → icon lookup, replacing the old array-position `icons[i]` indexing) and the admin icon-picker grid.
- **Validation**: `sanitizeHomepageContent()` — the actual API-facing validator (per the project's "no runtime validation on API bodies" risk, this endpoint gets real validation): caps `heroSlides` at 4 and `missionItems`/`whyjoinItems` at 6, drops items with an empty title, rejects unknown icon names (falls back to a default), truncates oversized strings. Unit-verified with 7 targeted cases (cap enforcement, unknown-icon fallback, empty-array → `undefined` so it falls back to defaults, garbage-input safety, string truncation) — all passed.
- **Public components** (`Hero.tsx`, `About.tsx`, `Mission.tsx`, `WhyJoin.tsx`): all gained new *optional* props that fall back to today's exact hardcoded/i18n behavior when unset — confirmed via live render that an association with no `homepageContent` produces byte-identical output to before this feature (same stock images, same default copy). Mission/WhyJoin's grid changed from a `grid-cols-3` (implicitly designed for exactly 6 items) to `flex flex-wrap justify-center` with per-card width classes, so 1-6 items all look balanced — including a genuinely partial last row (verified live with 3 mission items and a single why-join item).
- **API**: extended `PUT/GET /api/admin/branding` (the endpoint already built for `colorPreset`) to also accept/return `homepageContent`, validated via the sanitizer above.
- **Admin UI** (`/admin/settings`): 3 new tabs (About, Mission, Why Join) + the existing Hero tab extended with a slide-list editor (image upload + caption, add up to 4 / remove) sitting above its pre-existing generic `hero_image` field (left untouched, still works) + the Assets tab's `favicon_image`/`default_member_image` rows swapped from plain URL text inputs to the existing `ImageUpload` component (upload-or-paste-URL, already used in 5 other admin forms — this directly answers "shouldn't these be uploads?"). Mission/WhyJoin share one `ContentItemsEditor` (icon-picker + title + description per card, add up to 6/remove, explanatory copy when empty explaining 0 cards = defaults still showing). All 4 tabs share one `homepageContent` state object and a single save function, so saving one tab's edits never clobbers another tab's — verified this specifically since each tab edits a slice of the same JSON blob.
- **Small fix**: `app/(admin)/admin/dashboard/page.tsx` checklist item wording — "Add member venues" → "Add members" in person mode, using `member_mode` that was already being fetched on that page (zero new queries).

Verified: `npx tsc --noEmit` clean throughout, full `npm run build` clean. End-to-end tested via temporary DB writes (reverted after): set 2 custom hero slides, custom About image/headline/badge, 3 mission items, and a single (1-item) why-join card — all rendered correctly on the live homepage, old default copy correctly disappeared, then reverting to `NULL` confirmed the exact default content returns.

**Takeaway for future work**: this endpoint (`/api/admin/branding` PUT) is now the pattern for "per-association Association-model content" — reuse it (extend the accepted body + sanitizer) rather than routing new structured content through the generic `SiteSettings` KV endpoint, which only supports flat strings.

---

## Fix — Events Module Bug Sweep + Multi-Image Event Galleries ✓ COMPLETE (2026-08-30)

User screenshots from a live person-mode association (Namo:Udyam) surfaced a cluster of real bugs, mostly variations on the "hardcoded EVA Nepal" theme already fixed elsewhere this session, plus image-handling bugs and a real feature gap.

**Hardcoded "EVA Nepal" audit** (asked explicitly: "check where else such issues were missed"):
- `app/api/ai/generate/route.ts` — all 6 prompt types (bio, news, agenda, meeting, minutes, event) hardcoded "EVA Nepal (Event and Venue Association Nepal)" directly in the LLM prompt text. Now fetches the real association name once per request and interpolates it. Also fixed a leftover "upgrade your Gemini API plan" error message — the system uses Groq, not Gemini.
- `components/admin/EventForm.tsx` / `MemberForm.tsx` — the **client-side local template fallback** (`buildDescription()`, used when the AI call fails or is unreachable — this is what the screenshot actually showed, not the AI path) had "EVA Nepal" hardcoded in all 4 template variants of each. Both forms now fetch the real association name on mount (`/api/admin/branding`) and use it.
- `components/sections/Timeline.tsx` — missed during the earlier homepage-content pass. The "Our Journey" section's giant background year, "14 Years of Leading Nepal's Event Industry" headline, body copy, and bottom stat chips (Founded/Members/Years) were all fully hardcoded regardless of the real association — plus a 4th stat ("500+ Trained") was pure fabricated filler with no data source at all. Now takes `name`/`foundedYear`/`memberCount`/`yearsActive`/`memberMode` props (wired from `app/page.tsx`, all already computed there) with full mode-aware copy; the fabricated stat was dropped rather than fudged.
- Small copy fixes: `NewsForm.tsx` author-field hint, `CommitteeForm.tsx`'s "president" role description, `admin/login/page.tsx`'s copyright footer (was hardcoded despite the page already having the real `assocName` in state, just not using it).
- **Not a code bug**: the GROQ_API_KEY error is the AI route correctly reporting that production's environment is missing the key — this needs adding to the server's `.env` and a PM2 restart, not a code fix.

**Silent translate failures**: `EventForm.tsx`'s manual "Translate" button and `NewsForm.tsx`'s excerpt/content translate buttons caught fetch errors with no user feedback at all — a failed translation (e.g. missing API key) looked identical to a successful no-op. Added error state + a small inline message near each field. Left the *debounced auto-translate-while-typing* effects (Event/News/Committee forms) silent by design — surfacing an error on every keystroke pause would be noisy for a background convenience feature.

**`ImageUpload.tsx`** (shared by Member/Event/News/Committee/PersonMember forms — one fix covers all 5):
- Preview thumbnail used `next/image`, which round-trips through Next's `/_next/image` optimizer — this 400'd immediately after a client-side upload (confirmed via the reported browser console error), while working fine on a full page reload (edit mode) once the optimizer had "seen" the file. Switched to a plain `<img>`, matching the pattern already used everywhere else in this codebase for `/uploads/*` content.
- The upload fetch itself had **zero error handling** — `const { url } = await res.json()` with no `try/catch`, no `res.ok` check. A failed upload (network blip, server error) left `uploading` stuck `true` forever with no way to retry — the exact "stuck loading" bug class fixed extensively elsewhere this session, just missed in this one file. Added proper try/catch/finally + an error message.

**Event image display**: `EventCard.tsx` and the event detail page used `object-cover` on a fixed-height container, which badly cropped tall/portrait event flyers (confirmed against a real uploaded poster — the bottom half, including the "Register" CTA, was being cut off). Switched to `object-contain` (the card/detail containers already had a navy/slate background, so the letterbox fill was free). Also fixed the date badge on `EventCard.tsx`, which was plain text with no background chip — unreadable against certain photos — now a proper pill matching the other badges.

**Events page header made per-association editable**: `/events`'s "What We Do" headline and subtitle were global i18n copy (same as every other association in that mode). Added `eventsHeadline`/`eventsSubtitle` to the existing `HomepageContent` bundle (same pattern as Hero/About/Mission/WhyJoin), with a small editor added to the admin Hero tab.

**New feature — Promo/Recap event galleries**: user wants to upload photos before an event (3-4 promo images while upcoming) and after it completes (recap photos + an optional highlights video link). Added `Event.promoImages String[]`, `Event.recapImages String[]`, `Event.recapVideoUrl String?` (all additive, capped server-side at 4/6 images via `lib/event-gallery.ts`). Admin: multi-image add/remove editor in `EventForm.tsx`'s Details step, reusing `ImageUpload`. Public: the event detail page shows the promo gallery while upcoming, or the recap gallery + video (auto-embedded for YouTube/Vimeo links, plain link otherwise) once the event is past.

Verified: `npx tsc --noEmit` and full `npm run build` clean. Live end-to-end tests (temporary DB writes, reverted after): confirmed Timeline renders the real association name/years for `eva-nepal` and no hardcoded fallback leaks through for `namo-udyam`; confirmed a recap gallery + YouTube video correctly render on a real past event's detail page. One false alarm during verification worth noting: a `grep` initially "found" the old fabricated Timeline stat still live — turned out to be (a) a stray duplicate project directory literally named `" "` at the repo root left over from some past operation, unrelated to the running app, and (b) separately, a *different*, entirely legitimate per-milestone `TimelineEntry.stat` DB value that coincidentally read similarly. Flagged the stray directory to the user; did not touch it.

**Takeaway for future work**: `buildDescription()`-style client-side fallback templates (used when an AI/network call fails) are easy to forget when auditing for "association name" bugs, since they only fire in the *failure* path of the primary flow — worth explicitly checking for a local/offline fallback function whenever auditing a form's AI-generation feature, not just the server route.

---

## Fix — Member Detail Page Mode-Awareness, Groq Model Deprecation, Upload Compression ✓ COMPLETE (2026-08-30)

Follow-up to the events bug sweep — user asked to specifically re-check whether "venue" wording is properly gated by `member_mode` everywhere, plus raised the real GROQ_API_KEY error and a valid concern about uncompressed image uploads.

**More "venue" hardcoding found and fixed** (`app/members/[slug]/page.tsx` — the individual member profile page, missed entirely by the 2026-08-29 "public homepage respects member_mode" pass, which only touched the homepage sections): metadata title/description/keywords, "Featured Venue" badge, "Venue Details"/"Venue Type" section, "Other Venues in {area}", and the About-member body sentence were all 100% hardcoded to venue wording regardless of association mode — this page never fetched `member_mode` at all. Now fetches `getSettings()` like the members-list page does and branches all of the above. Also found and fixed two remaining ungated strings in `StatsSection.tsx` ("Kathmandu's largest venue network" and "Serving Nepal's event industry since…") that the ternary-gating pass had missed on two of the four stat cards. Verified live: `namo-udyam` (person mode) now shows "Member Details"/"Featured Member"/"Other Members in…"/"largest professional network"; `eva-nepal` (venue mode) confirmed byte-for-byte unchanged.

**Groq AI generation — real root cause found, not a config problem**: user correctly pushed back that GROQ_API_KEY was already in `.env` and had worked before. Confirmed via a direct `GET /v1/models` call against the real key that Groq has fully deprecated the model this route used (`llama-3.3-70b-versatile` — removed from the catalog entirely, not merely renamed). Its 400 error response body contained the substring `"invalid_request_error"`, which the route's error classifier's naive `message.includes("invalid")` check misread as a bad API key — a real key working correctly, misreported as "not configured." Fixed both: swapped to `openai/gpt-oss-120b` (verified working against the real key with a live completion), and replaced the substring-matching error classifier with typed error classes (`GroqAuthError`/`GroqQuotaError`) so only an actual HTTP 401 is ever reported as a key problem — any other 4xx/5xx now surfaces its real message instead of being silently misclassified.

**Image upload compression** — user flagged the real risk of unbounded high-resolution uploads accumulating on the server. Confirmed: `/api/upload` had zero resizing, zero format conversion, and zero size limit; `public/uploads` already held 40MB across just 24 files, 19 of them over 1MB (one single 2.97MB PNG). Rewrote the route to pipe every upload through `sharp` (already a project dependency, no new package needed): re-encodes to WebP at quality 80, downscales to a 1920px max dimension (`fit: "inside"`, never upscales), applies EXIF rotation before resizing, and rejects non-image MIME types plus anything over 15MB before processing. Verified directly against two real files already in `public/uploads`: 2.97MB → 66KB (97.8% reduction) and 2.42MB → 218KB (91.2% reduction) — both with resolution well within what any part of this app actually displays.

**Confirmed also-changeable, not a new gap**: Hero section placeholder images were already made admin-editable in the prior "Per-Association Homepage Content" pass (`heroSlides` in `HomepageContent`, `/admin/settings` → Hero tab) — re-confirmed this still covers the case the user was asking about; no further change needed.

**Housekeeping**: found and deleted (after review, with explicit user confirmation it wasn't intentionally created) a stray directory literally named `" "` at the repo root — a stale May 8 pre-multi-tenant snapshot of the project (missing `(platform)`/`(portal)`/`committee`/`meetings`/`opengraph-image.tsx`, all built later), not a git repo of its own, not referenced by any symlink or running process. This was the source of the false-alarm `grep` match noted in the previous entry.

Verified: `npx tsc --noEmit` and full `npm run build` clean throughout. Live end-to-end tests via temporary dev servers on both `namo-udyam` and `eva-nepal` for the mode-awareness fixes; direct `curl` calls against the real Groq API confirming both the root cause and the fix; direct `sharp` compression tests against real files already on disk.

**Takeaway for future work**: when a third-party LLM API starts erroring with "not configured"/"invalid key"-sounding messages that used to work, check `GET /v1/models` (or the provider's equivalent) against the real key before assuming the key is the problem — providers deprecate/rename models without warning, and generic substring-based error classification will misreport a model issue as a credentials issue. Prefer typed error classes over `message.includes(...)` for this kind of branching.

---

## Fix — nginx Upload Limit, AI Translation Hallucination, Event Gallery Lightbox ✓ COMPLETE (2026-08-30)

Live production screenshots surfaced four more issues, all traced to real root causes.

**nginx blocking uploads entirely (413)**: the "broken image" and "what is the upload limit?" reports traced back to the same root cause — nginx's default `client_max_body_size` (1MB) was rejecting image uploads with a raw, unstyled "413 Request Entity Too Large" page *before the request ever reached the Next.js app or its 15MB app-level check*. The admin had worked around this by pasting a Google Images thumbnail URL instead (visible in the screenshot's image-URL field), which is exactly the kind of fragile hotlinked URL that breaks (confirmed broken in a later screenshot). Added `client_max_body_size 20m;` to `nginx.conf`'s server block(s) — **this requires a manual step on the actual server**, since this repo's `nginx.conf` is a reference template and the live server's real config (serving multiple subdomains now) may not exactly mirror it; add the same directive there and `sudo nginx -t && sudo systemctl reload nginx`. Also added a matching client-side check in `ImageUpload.tsx` so an oversized file is caught instantly (no round-trip) with a message stating the actual 15MB limit, a specific message when the server does return a 413, and a permanently-visible "Max 15MB — automatically compressed on upload" hint so admins know the limit without hitting the error at all.

**AI translation inventing content not in the source text**: reported as "Nepali translation ... is using EVA Nepal and not converting the information provided" — confirmed via a direct Groq API call using the exact reported input that the model was inserting "इभा नेपाल द्वारा आयोजित" ("organized by EVA Nepal") into a Nepali translation of English text that never mentions any association name, hallucinated by the model itself (not from the app's prompt, which was already generic). Root cause: the translate prompt was a loose one-liner ("Translate the following text...") running at the same 0.7 temperature used for creative generation tasks, giving the model room to embellish. Fixed by (a) rewriting the prompt with explicit strict-fidelity rules ("do NOT add any information/names/organizations not in the source", "if the source names no organization, the translation must not introduce one"), and (b) adding a `temperature` parameter to `callAI()`, called at `0.2` for the translate type only (all other generation types keep 0.7, which is appropriate for genuinely creative tasks). Re-verified against the real Groq API with the exact previously-hallucinating input — output is now a faithful translation with no invented content.

**Event gallery UX** — three related fixes to `app/events/[slug]/page.tsx`'s promo/recap gallery display, reported via screenshots showing a broken thumbnail, images opening in a new browser tab, and confusion about where the promo gallery had gone:
- Replaced the plain `<a target="_blank">` grid with a new `components/ui/ImageLightboxGallery.tsx` (client component): a horizontally-scrollable snap-scroll thumbnail strip that opens a full-screen in-page lightbox modal on click, with prev/next arrow buttons, keyboard arrow-key navigation, Escape-to-close, and body-scroll lock — same interaction pattern as the existing `CommitteeMemberModal.tsx`. Used for both the promo gallery (shown while upcoming) and recap gallery (shown once past).
- **Root cause of "where did my promo gallery go?"**: working as designed (promo shows only while upcoming, recap only once past — the user's own original spec), but the event in question had already crossed into "past" status by the time it was tested (its date had passed relative to today), so the promo images the admin uploaded had nothing displaying them. Fixed with a safety-net fallback: if an event is past and has no recap images set, the promo gallery is shown instead (labeled "Event Gallery") rather than the uploaded photos just disappearing.
- Added always-visible (not just hover-tooltip) inline explanatory text under each gallery section in `EventForm.tsx` stating exactly when it displays publicly, so this isn't a surprise next time.

Verified: `npx tsc --noEmit` and full `npm run build` clean. Live end-to-end tests via temporary dev server + direct DB writes (reverted after): confirmed the promo-fallback correctly labels itself "Event Gallery" (not "Event Recap") when recap images are empty on a past event, and confirmed the new lightbox markup (scroll-snap thumbnail strip, no more `target="_blank"` links) renders in place of the old grid.

**Takeaway for future work**: any LLM call intended to be *faithful to its input* (translation, summarization-without-embellishment, format conversion) should use a low temperature (~0.1–0.3) and an explicit no-invention instruction — the default "creative" temperature used for open-ended generation tasks is exactly the wrong setting for these, and the failure mode (silently inventing plausible-sounding but wrong content) is much harder to notice than an outright error.

**Same-day follow-up — raw upload ceiling lowered 15MB → 5MB**: user questioned whether 15MB was too generous and asked to confirm compression was actually happening. Clarified the distinction that matters: compression already caps the *stored* file size regardless of the raw ceiling (already-verified 90-98% reduction applies at any input size), so the ceiling's only real job is bounding memory/CPU during processing on a memory-constrained VPS (`--max-old-space-size=180` in `ecosystem.config.js`). Lowered to 5MB (`app/api/upload/route.ts`, `ImageUpload.tsx`'s matching client-side pre-check, and `nginx.conf`'s `client_max_body_size` correspondingly to `6m`) rather than 2MB as the user first floated — a typical modern phone photo is 2-8MB raw, so 2MB would start rejecting normal legitimate photos and reintroduce the same confusing "why won't my upload work" experience just fixed by the nginx change above.
...
## Fix — Hero Slide Text Overflow (thumbnail captions, accent line, slide indicator) ✓ COMPLETE (2026-08-31)

Screenshot report: on the homepage Hero carousel, a long custom Hero-slide label ("Startup exhibitions. Founder stories. Panel discussions. Networking. Pitch sessions.") was visibly overlapping/colliding with other elements — spilling out of the floating thumbnail card into the "Member Venues" stat badge, and spilling out from the bottom-right slide indicator into the stats bar row above it.

Root cause: `Hero.tsx` reuses a single Hero-slide `label` string (the only text field `HeroSlide` has, per `lib/homepage-content.ts`) across **three** different UI slots that were each designed for short text — the small-caps "accent" kicker line above the H1, the bottom-right slide-name indicator, and both floating thumbnail captions — with zero length constraints anywhere. A short label ("Grand Banquet Halls") fits fine in all three; a full sentence does not, and none of the three had `truncate`/`line-clamp`/`max-w` to degrade gracefully.

Fix: added `truncate max-w-[220px]` to the slide indicator, `line-clamp-1 max-w-xl` to the accent line, and `line-clamp-2` to both thumbnail captions — a long label now truncates cleanly with an ellipsis in every slot instead of overflowing into neighboring elements. Reproduced the exact reported collision locally (temporary DB write matching the reported label text, reverted after) and confirmed visually that both the thumbnail-badge collision and the slide-indicator/stats-bar collision are gone.

**Separate finding, not fixed (flagged for follow-up, out of scope for this report)**: while investigating, found that the Hero's small-caps "accent" line and the bottom-right slide-name indicator are actually invisible on every single page load — both `motion` elements (the ones using `key={current}` to re-animate on slide change) get stuck at their `initial` state (`opacity: 0`) and never reach `animate`, reproduced identically in a real production build (`next build && next start`), with the 50ms progress-timer re-render ruled out as a cause, and even with `AnimatePresence` removed entirely. Every OTHER Hero animation (badge, H1 titles, CTAs, stat cards — none of which use a `key` prop) animates correctly. This looks like a real, pre-existing, sitewide bug isolated to the `key={current}`-based elements, unrelated to today's overflow fix and not something the user actually reported (they saw overlapping visible text, not missing text) — needs its own investigation session before touching it, since the root cause wasn't conclusively identified.

---

## Fix — Site-wide Hash-Anchor Navigation Broken (Committee, About, Contact, Join links) ✓ COMPLETE (2026-08-31)

User reported the navbar "Committee" link didn't navigate. Investigation found this was NOT specific to Committee at all — every `/#anchor`-style nav link (About, Committee, Contact, Join) across the whole public site was broken in two independent, compounding ways:

1. **Clicking did nothing at all.** `next/link` (`<Link href="/#committee">`) simply failed to trigger navigation when clicked from a page other than the homepage — the URL never even changed. Root cause not fully isolated (a known class of Next.js App Router bug around `<Link>` + hash-only-differing hrefs), but the fix sidesteps it entirely: every hash-anchor link (`Navbar.tsx` desktop/mobile nav + both "Join" buttons, `Footer.tsx` Quick Links, `Hero.tsx`/`WhyJoin.tsx`'s "Join"/"Apply" buttons, `committee/history/page.tsx`'s "Back to current committee" link) now renders a plain `<a href="/#...">` instead of `<Link>` — real anchor tags trigger genuine, reliable browser navigation in every case (same-page instant jump, cross-page full navigation that lands on the right hash), which is exactly the native behavior anchor links have always had and next/link's client router doesn't reliably reproduce here.
2. **Even a real, successful navigation to the hash didn't scroll to it.** Confirmed via direct URL load (`/#committee`) that the browser's own native hash-scroll-on-load silently fails to land on a section far down this heavily client-rendered homepage — it only attempts the scroll once, before the target has actually mounted, and never retries. Fixed in `PublicChrome.tsx` (the shared wrapper around every public page) with an explicit scroll handler: a `MutationObserver` waits for the target element to actually appear (no matter how long that takes, up to a 10s safety timeout) rather than a fixed-count retry loop, then scrolls to it with a `HASH_SCROLL_OFFSET` so the fixed navbar doesn't cover the section heading.

**A second, harder-to-find bug was hiding inside that scroll fix**: even once the target element was found immediately, `window.scrollTo({ top, behavior: "smooth" })` never actually moved the page — extensive live debugging (screenshots + DOM checks, since a genuine tooling quirk in the verification browser made raw scrollY reads unreliable at one point and had to be worked around by checking `document.elementFromPoint` instead) traced this to the homepage Hero carousel's autoplay progress-bar timer, which re-renders the whole Hero component every 50ms — each of those repaints was silently cancelling the in-flight smooth-scroll animation before it ever completed, so the page stayed pinned at the top no matter how many times the scroll was reasserted. Switched to `behavior: "instant"` (a non-animated scroll can't be interrupted mid-flight) — confirmed working end-to-end afterward.

Verified live (production build, `next build && next start`) via all three real paths: navbar "Committee" link from `/events` → lands on the Executive Committee section; navbar "About" link from `/events` → lands on the About section; Committee History page's "Back to current committee" link → lands back on the Executive Committee section. `npx tsc --noEmit` and full `npm run build` both clean.

---

## Fix — Images Broken on Initial/Cold Load, Sitewide ✓ COMPLETE (2026-08-31)

User reported broken images specifically on first paint / first-time visits, across BOTH the public homepage (Hero, cards) AND the admin/portal/platform panels (logos, upload previews). Investigation (background agent, read-only) found the root cause: every remaining `next/image` `<Image>` usage routes through Next's **on-demand image optimizer** (`/_next/image`), which generates each image+size variant lazily on first request — and this exact failure mode was already proven once this session (`PROGRESS.md`, `ImageUpload.tsx` fix: the optimizer 400'd immediately after a fresh upload, working fine only after a full reload once it had "seen" the file). That earlier fix only touched the upload-preview widget; every other logo and two member-photo call-sites still carried the identical risk — and critically, **every association/admin/portal logo across the whole site shares this exposure simultaneously**, since they all resolve through the same `/_next/image` code path on every cold request.

Fix: removed `next/image` entirely — grepped the whole repo down to zero remaining `from "next/image"` imports. Every logo (public `Navbar`/`Footer`, admin `AdminShell` sidebar, portal `PortalShell` sidebar, the shared `AuthCard` used by portal+platform login, admin login page, both static "Powered by Nibjar" credits) now renders as a plain `<img>` — bypassing the optimizer entirely, which is also strictly correct for these since they're either static build assets or already-compressed uploads (`/api/upload` already runs everything through `sharp` at upload time — double-processing through the optimizer was never buying anything). The two remaining member-photo call-sites (`app/committee/history/page.tsx`'s inline card, `app/members/[slug]/page.tsx`'s profile photo) were upgraded to `SmartImage` instead of a bare `<img>`, consistent with the rest of the photo-card rollout, since they benefit from the skeleton/retry/fallback treatment too.

Investigation also flagged a plausible **secondary, compounding cause** worth knowing about but not changed here (a server-ops decision, not app code): `ecosystem.config.js`'s PM2 config caps the process at `--max-old-space-size=180` / `max_memory_restart: "200M"`, a tight ceiling for a process also running Prisma + NextAuth + `sharp`-based upload compression — a memory spike during either could trigger a PM2 restart that aborts in-flight requests (including images), self-healing moments later. Worth watching `pm2 logs` for restart correlation if broken-image reports persist after this deploy.

Verified live (production build): confirmed via `read_network_requests` that a full cold homepage load now makes **zero** `/_next/image` requests — every logo and upload loads as a direct static-file `GET` (200, no optimizer round-trip). Also visually confirmed the admin login page's logo, both Nibjar credits, and all of the previous round's fixes (description, "Based in") still render correctly together. `npx tsc --noEmit` and full `npm run build` both clean.

---

## Feature — Duplicate Uploads Detection + Merge (admin panel + cron) ✓ COMPLETE (2026-08-31)

Follow-up to the unused-uploads cleanup: user asked whether the same cron also removes *duplicate* uploads (same photo uploaded more than once for different fields, e.g. an association's logo and its "default member image" set to the same file), noting duplicates "can be used in all places" — i.e. any merge has to be safe even when several different records point at the copies being consolidated.

Discussed the key difference before building (LAW 1): deleting an *unused* file is always safe — nothing points to it. Merging a *duplicate* means picking one copy as canonical, **rewriting every DB record** that pointed at the other copy to point at the canonical one instead, then deleting the now-redundant file — a live write across every image-holding table, not just a file delete. Since `public/uploads` is shared across every association on the platform, two different associations could independently upload a byte-identical stock photo; merging those would silently couple their records to the same physical file. User confirmed: (1) automation level — build **both** a reviewable admin-panel flow (scan → preview → confirm, same pattern as the unused-images panel) and wire it into the unattended cron, and (2) scope — only merge duplicates where every reference resolves to the **same association**; cross-association duplicates are detected and reported but never auto-merged.

- **`lib/uploadsCleanup.ts`** extended with `mergeDuplicateUploads()`: hashes every file in `public/uploads` (SHA-256, streamed) to find byte-identical groups, then — for each group — collects every DB reference to every file in it via a new `collectFileReferences()` (a richer version of the existing reference-scanner: instead of just a `Set` of referenced filenames, it returns each occurrence as a `{ associationIds, describe, rewrite }` closure covering every image-holding field, including array fields like `Event.promoImages`/`recapImages` by index and nested JSON like `Association.homepageContent.heroSlides[].image`). If every reference across the whole group resolves to one association (or none), it's merged: canonical = the file with the most existing references (ties broken by earliest upload), every other file's references get rewritten via their `rewrite()` closure, then the now-unreferenced duplicates are deleted. Groups spanning more than one association are left untouched and returned separately as `skippedCrossTenant`. Same 24-hour grace period as the orphan cleanup, and a `dryRun` mode for preview.
- **Admin UI**: new "Duplicate Images" panel on the same Platform → Storage page, right below the existing "Unused Uploads Cleanup" panel — identical scan-preview-confirm UX, plus a distinct amber notice listing any cross-association groups that were found but intentionally not merged.
- **API**: same `POST /api/platform/uploads-cleanup` route, now takes an optional `mode: "duplicates"` to route to the merge function instead of the orphan cleanup (defaults to the existing orphan behavior for backward compatibility).
- **Cron**: `scripts/cleanup-uploads.ts` now runs both passes in the same run — orphan cleanup first, then duplicate merge — logging a one-line summary of each plus which files were kept/removed/skipped.

**Verified with real DB writes, not just a dry-run read**: built synthetic same-association and cross-association duplicate pairs (backdated past the 24h grace period), ran a dry-run and confirmed the JSON output was exactly right (including correctly grouping a synthetic pair with a real pre-existing identical file), then ran the actual merge and directly queried Postgres + the filesystem afterward to confirm the referencing record's field was rewritten to the canonical filename and the duplicate file was deleted — while the cross-association pair was left completely untouched on disk. This also surfaced and correctly merged two genuine pre-existing duplicate pairs already sitting in the local dev data. Also exercised the full flow through the real admin UI (temporary local-dev-only platform-login password, per the established testing pattern) and confirmed the scan preview and the "Merged 1 duplicate file..." success state both render correctly. All test rows/files cleaned up afterward. `npx tsc --noEmit` and full `npm run build` both clean.
...
## Fix — Images Stuck/Broken in PWA (but fine on Web) + Recap Gallery Not Swipeable on Mobile + Logo Auto-Retry ✓ COMPLETE (2026-09-01)

User reported (4 screenshots from their phone, real PWA + mobile Safari) that images were still broken in the PWA/mobile view even though a prior session's `next/image` removal had fixed general image loading on web, and that the Event Recap gallery — a working horizontal slider on web — wasn't swipeable on mobile at all.

**Investigated on the live production site first** (not local dev) rather than assuming it was the same old missing-uploads incident — confirmed via direct network/DOM inspection that the specific image in the report ("Learn Investing" event cover) was NOT missing: the browser reported it as fully loaded (`complete: true`, real `naturalWidth`) while the page still showed a blank/stuck placeholder over it. That ruled out "still-missing file" and pointed at a genuine rendering bug instead.

**Root cause 1 — `components/ui/SmartImage.tsx`, cached-image race condition**: a browser- (or service-worker-) cached image can finish loading and fire its native `load` event *before* React finishes attaching the `onLoad` listener — especially likely in a PWA, since `sw.js` serves static images cache-first on every repeat visit, making near-instant cache hits the common case rather than the rare one. When that race is lost, `onLoad` never (re)fires, so `status` stays stuck at `"loading"` forever and the shimmer placeholder covers an image that actually loaded fine. This explains why it looked "fixed on web" (a fresh tab, first visit, fetches over the network — slow enough that React's listener is reliably attached before load completes) but consistently broken in the PWA (SW cache-first makes the race trigger far more often). **Fix**: added a `useEffect` with no dependency array that checks `imgRef.current?.complete` after every render and self-corrects `status` to `"loaded"` if the native element already finished — catches exactly the case the `onLoad` prop misses.

**Root cause 2 — `app/globals.css`, `body { overflow-x: hidden }` blocking nested touch-scroll**: a known WebKit/iOS quirk where `overflow-x: hidden` on `<body>` can prevent touch-swipe gestures from reaching a *nested* horizontally-scrollable element, while leaving desktop mouse/trackpad scrolling on that same element completely unaffected — exactly matching "worked as a slider on web, not swipeable on mobile." **Fix**: `components/ui/ImageLightboxGallery.tsx`'s thumbnail strip now explicitly declares `touchAction: "pan-x"` (plus `WebkitOverflowScrolling: "touch"`) so iOS routes horizontal swipes to it directly instead of leaving the gesture ambiguous.

**Follow-up, requested by user after seeing the above**: noticed while investigating that the site's logo (navbar, footer, admin/portal sidebars, admin login page, `AuthCard`'s brand mark) was still a plain `<img>` with zero error handling — unlike every other dynamic image on the site, which already has retry logic. A network blip reopening the PWA would leave the logo permanently broken with no recovery. User asked for the same auto-retry treatment, but with a **placeholder image on final failure instead of a "tap to retry" button** (a logo isn't something a visitor would think to interact with). Built `components/ui/LogoImage.tsx` — same 2-retry/1.5s-delay pattern as `SmartImage`, but renders a plain unwrapped `<img>` (no sized/positioned wrapper div, so a logo's natural "fixed height, auto width" sizing is unchanged) and swaps to `/default-logo.png` (the existing generic fallback logo, already used elsewhere for "no logo configured") on final failure instead of showing a retry button. Wired into all 7 real logo call-sites: `Navbar.tsx`, `Footer.tsx`, `AuthCard.tsx`'s `brandMark()`, `PortalShell.tsx`, `AdminShell.tsx`'s `LogoMark()`, and both logo spots in the admin login page. The static bundled "Powered by Nibjar" credit logos were left as plain `<img>` — they're same-origin build assets, never subject to upload-network flakiness.

Verified live: `npx tsc --noEmit` clean throughout. Confirmed the fallback pipeline end-to-end in a real browser — with `Association.logo` empty, `LogoImage` correctly resolved through its retry attempts to `/default-logo.png` and rendered it cleanly (`complete: true`, real `naturalWidth`), no broken-image icon at any point. The `SmartImage` cached-race fix and the gallery `touch-action` fix are both PWA/iOS-specific timing and touch-gesture behaviors that can't be fully reproduced in a desktop test browser — flagged to the user as needing on-device confirmation after deploy.

---

## Feature — Pull-to-Refresh Gesture for Installed PWAs ✓ COMPLETE (2026-09-01)

Follow-up to the image/gallery fixes above: user pointed out there's still no way to reload/refresh inside the installed PWA on mobile — a real gap, since installed PWAs have no browser chrome at all (no URL bar, no native pull-to-refresh, which is a browser-tab-only feature) and iOS resumes a backgrounded PWA from memory rather than performing a fresh navigation, so stale content had no recovery path short of force-quitting the app (documented earlier this session).

Built `components/ui/PullToRefresh.tsx` — a self-contained gesture handler: tracks touch position only when `window.scrollY === 0`, applies a resisted (`×0.5`) downward transform to the wrapped content as the user pulls (revealing a small rotating spinner above it, capped at 90px of pull), and calls `window.location.reload()` if released past a 70px threshold. Deliberately gated to only activate when `window.matchMedia("(display-mode: standalone)").matches` (or `navigator.standalone` for older iOS) — i.e. only inside an installed PWA. A normal browser tab already has its own native pull-to-refresh; running a custom one on top of that would just be a confusing double gesture, and `enabled` staying `false` there means zero listeners are attached and children render completely unwrapped (no perf/behavior cost in the common case).

Wired into all three real user-facing shells: `PublicChrome.tsx` (public site), `AdminShell.tsx`, and `PortalShell.tsx` — all three are separately-installable PWAs per the 4-manifest architecture, and the user's own screenshots showed the issue in both the public site and the admin sidebar. `PlatformShell.tsx` intentionally left as-is (no user report there, smaller/internal audience).

Verified live: confirmed zero visual/behavioral change in a normal (non-standalone) browser tab first — `enabled` correctly resolves `false`, no pull indicator in the DOM. Then temporarily hardcoded `setEnabled(true)` (reverted immediately after) to simulate standalone mode, since Chrome's own tabs can't be forced into `display-mode: standalone` from these tools — dispatched synthetic `TouchEvent`s to simulate a real pull gesture: confirmed the indicator appears with correct opacity/rotation/transform math mid-pull (screenshotted), and confirmed release past threshold actually calls `window.location.reload()` (the test script's own execution was interrupted by the resulting navigation — direct proof the reload fired). Re-checked the admin panel's layout after wiring to confirm no regression. `npx tsc --noEmit` clean throughout.

---

## Fix — AnimatedSection Scroll-Reveal Getting Visibly Stuck Mid-Fade (root cause of "layout spreading/broken" reports) ✓ COMPLETE (2026-09-01)

User reported (3 screenshots, mobile web) that the homepage Upcoming/Past Events section looked visually broken "when zoomed out" — content spread/misaligned — on both the Upcoming and Past sides, plus a third report that the event detail page "still isn't resolved." Pointed and fair frustration ("how are you missing such design failures") given prior sessions had already fixed several image-loading issues.

**Actually reproduced this time, not just theorized from screenshots**: `resize_window` doesn't reliably change a tab's real rendering width in this environment (confirmed stuck at desktop width despite reporting success) — worked around it by injecting a same-origin `<iframe>` with an explicit CSS `width: 390px` into the page via `javascript_tool`, which genuinely renders at that width (`iframe.contentWindow.innerWidth` confirmed ~386px) regardless of the outer browser window. Used that to jump-scroll straight to the Events section on the live production site and screenshotted immediately: caught multiple event cards with fully-opaque borders/badges but their inner text still at very low opacity — a genuine, direct capture of the reported symptom, not a guess.

**Root cause**: `components/ui/AnimatedSection.tsx` (used site-wide — About, Events, WhyJoin, Mission, Committee, etc.) gates every wrapped element behind Framer Motion's `useInView(ref, { once: true })`, with callers passing an uncapped per-item stagger delay (e.g. `delay={i * 0.1}` in `Events.tsx`, `delay={i * 0.08}` elsewhere). Under normal gradual scrolling this is invisible — each card enters the viewport and animates in one at a time, spaced out naturally. But a **bulk viewport change** (pinch-zooming out, which reveals a large swath of previously off-screen content all at once; an instant hash-jump; a fast programmatic scroll) fires many of these `useInView` triggers within the same frame, and their cumulative stagger delay (10th item = 1s+ before its 0.6s transition even starts) leaves a visibly inconsistent, half-rendered page for 1-2+ seconds — badges/borders fully drawn (not gated by the animation) next to barely-visible text (gated), reading exactly as "spreading"/broken layout to anyone who screenshots or judges the page during that window. This is the same underlying bug *class* as the already-known, still-unsolved Hero.tsx issue (Framer Motion elements stuck at their `initial` state) — different component, same root fragility (scroll-triggered animation reliability), now confirmed to affect the shared component most of the site's sections are built on, not just Hero.

**Fix, in `AnimatedSection.tsx` itself** (fixes every call site at once, no per-page changes needed):
1. Clamp the incoming `delay` prop to a 0.4s ceiling (`Math.min(delay, 0.4)`) — bounds the worst-case cumulative stagger regardless of how many items a caller staggers.
2. Added a 1.5s fallback timer: if the IntersectionObserver behind `useInView` hasn't fired by then, force the content visible anyway. This is the direct fix for the *permanently*-stuck failure mode (not just the "slow to settle" one) — the same class of bug Hero.tsx has been carrying unsolved.

**Not fixed / separate**: Hero.tsx's own stuck elements use raw `motion.p`/`motion.span` with `key={current}`, not this shared component, so this change doesn't touch that specific case — it remains a separate, still-open item (see PROGRESS.md's earlier "Hero Slide Text Overflow" entry).

**Event detail page ("still not resolved")**: grepped `app/events/[slug]/page.tsx` and its sub-components — confirmed they don't use `AnimatedSection`/`whileInView` at all, so this fix doesn't apply there, and nothing in the corresponding screenshot showed a visible defect (cover image loaded, gallery thumbnails loaded, text fully legible). Asked the user directly what specifically still looks wrong there rather than guessing further or claiming a fix that isn't verified.

Verified: `npx tsc --noEmit` clean. Live-tested the actual fix (not just the bug) via the same iframe-at-real-width technique against the local dev server — jump-scrolled to the About section (uses the same `AnimatedSection` pattern with staggered tile delays) and confirmed everything renders fully opaque immediately after an instant scroll, no transient fading. DEPLOYED 2026-09-01 (same deploy batch as the P/Q PWA fixes — see MEMORY.md build-phase log).

---

## Follow-up — Extended the Scroll-Reveal Fix to Every Direct `whileInView` Usage, Not Just AnimatedSection ✓ COMPLETE (2026-09-01)

Immediate follow-up to the `AnimatedSection` fix above. User asked directly: "is every design flaw fixed? in web, admin portal and member portal?" — a fair question given the same bug class had already slipped through once (Hero.tsx, still unsolved) before being caught a second time (Events section, just fixed). Rather than answer from memory, audited the actual codebase.

**Findings**:
- Admin panel and Member Portal: `grep` for `whileInView`/`AnimatedSection` across `app/(admin)`, `app/(portal)`, `components/admin`, `components/portal` returned zero matches — this animation pattern doesn't exist there at all, so this specific bug class cannot occur on those two surfaces. A clean, verifiable "no" to part of the question.
- Public site: 4 more files (`News.tsx`, `Events.tsx`, `StatsSection.tsx`, `Timeline.tsx`) use `whileInView`+`viewport` **directly** on their own `motion.div`s instead of going through the just-fixed `AnimatedSection` wrapper — meaning the fix didn't actually cover them. Their stagger delays turned out to be naturally bounded already (multiplied against small `.slice()`-capped list lengths — 3, 5, 6 items — so max cumulative delay was already under the 0.4s ceiling), but every one of them was still missing the "force visible if the observer never fires" safety net.

**Fix**: extracted the safety-net logic out of `AnimatedSection.tsx` into a shared `components/ui/useSafeInView.ts` (`useSafeInView(margin)` hook + `clampStagger(delay, max)` helper), and applied it to all 10 remaining direct `whileInView` call sites across the 4 files — converting each from the `whileInView`/`viewport` shorthand to `ref`+`animate={isInView ? shown : hidden}`. Three of these (`Events.tsx`'s `UpcomingEventCard`/`PastEventRow`/the Event Types legend chip) required extracting a small named component first since hooks can't be called inside a `.map()` callback — the legend chip became a new `EventTypeChip` component, the two others already had their own components. `AnimatedSection.tsx` itself was refactored to consume the shared hook too, removing the duplication.

**Deliberately left alone**: `Timeline.tsx`'s one remaining `whileInView` (line ~185) animates a purely decorative vertical progress line (`scaleY: 0 → 1`) that doesn't gate any readable content — worst case if it never triggers is a flat decorative line, not hidden text. Not part of the reported bug class, left as-is per LAW 2 (don't touch working things beyond what's actually broken).

**Still open, unrelated pattern**: Hero.tsx's stuck elements use raw `motion.p`/`motion.span` with `key={current}` (remount-based, not `useInView`-based) — a different mechanism entirely, not covered by any of this work. Remains open.

Verified: `npx tsc --noEmit` clean. Live-tested the refactored `StatsSection.tsx` on local dev (Events/News have no local test data — local DB is empty for those) via an instant jump-scroll: all 4 stat cards, headline, and tagline rendered fully opaque immediately, no fading, no console errors. DEPLOYED 2026-09-01 (same deploy batch as the P/Q PWA fixes — see MEMORY.md build-phase log).

---

## Fix — Real Horizontal Overflow on Mobile ("blank white space when zoomed out") ✓ COMPLETE (2026-09-01)

User reported (again, with Chrome DevTools screenshots this time: iPhone 14 Pro Max 430×932 at "0%" zoom) that Upcoming/Past Events cards were "expanding to the section rather than fitting in," visible as blank white space to the right of the content and a spread-out footer. My first response wrongly guessed stale caching — user pushed back hard with the actual DevTools evidence, which was correct and I had not taken seriously enough. Re-investigated properly this time.

**Root causes found, all real and independently reproduced** (via same-origin `<iframe>` injected with an explicit `style.width`, since `resize_window` doesn't reliably change a tab's true `innerWidth` in this environment; `iframe.contentWindow.scrollWidth - innerWidth` measured directly):

1. **Oversized decorative background blobs.** `ExecutiveCommittee.tsx`, `StatsSection.tsx` (×2 blobs + a `text-[20rem]` watermark), `WhyJoin.tsx` (×2 blobs) all used large (400-600px) absolutely-positioned glow blobs, several intentionally translated half off their section's edge (`translate-x-1/2`) for a corner-glow look. `overflow-x: hidden` on `html`/`body` hides these visually but does **not** shrink what `scrollWidth` reports — confirmed directly (`scrollWidth` identical whether `overflow-x` was `hidden` or `clip`). That "invisible" extra width still counts as real overflow, and becomes visible blank space whenever a mobile browser's zoom/scale recalculates layout — exactly what the user's DevTools screenshots showed. **Fix**: made all these blobs responsive (small at mobile/`sm:`, full size only at `lg:`) and removed their edge-crossing `translate-x` at mobile/tablet sizes, applying it only at `lg:` where there's room to spare.
2. **CSS Grid `min-width: auto` trap.** `Events.tsx`'s `grid lg:grid-cols-3` (Upcoming/Past columns) and `MembershipForm.tsx`'s `grid lg:grid-cols-2` (Info/Form columns) never set `min-w-0` on their grid item wrappers — grid/flex items default to `min-width: auto`, refusing to shrink below their content's intrinsic width even in a single-column mobile layout, which can force the whole grid (and page) wider than the viewport. This was the primary, most impactful bug — verified 54-475px of overflow eliminated exactly. **Fix**: added `min-w-0` to the grid item wrapper `div`s in both files.
3. **Horizontal slide-in animation transform.** `About.tsx` and `MembershipForm.tsx` both used `<AnimatedSection direction="left">` on their right-hand grid column. `AnimatedSection`'s `directionMap` gives `direction="left"` an initial/pre-animation `x: 40` transform offset — while an element sits in that state (either genuinely mid-animation, or during the ~1.5s `useSafeInView` fallback grace period before a stalled `IntersectionObserver` is forced visible), that 40px `translateX` is a real, measurable contributor to `scrollWidth`, unlike a vertical offset which can't affect horizontal overflow. **Fix**: changed both instances to `direction="up"` (vertical-only offset) — preserves the fade/slide entrance effect with zero horizontal-overflow risk. Left `AnimatedSection.tsx` itself untouched since it's used correctly (vertical directions only) everywhere else on the site.

**Also**: `globals.css` changed `overflow-x: hidden` → `overflow-x: clip` on `html`/`body`. Confirmed this does *not* change what `scrollWidth` reports (tested directly, see above) — kept anyway as the stricter/more correct value for blocking programmatic horizontal scroll, but it was **not** the actual fix; the fixes above are.

**Verified rigorously** (given the trust cost of the earlier wrong "caching" diagnosis) via the local dev server: fresh iframe load at a genuine 426px `innerWidth`, full-page sweep in 300px scroll increments (43 checkpoints spanning the entire ~12,769px page) — **zero overflow found anywhere**, including Hero.tsx's separately-noted 12px overflow (also resolved, apparently by the same blob/grid fixes). Re-checked About/MembershipForm/Events/Committee individually, immediately after scroll (no settle time) to catch any transient mid-animation state — all clean. Confirmed via computed-style inspection that About's right column transform changed from `matrix(1,0,0,1,40,0)` (translateX) to `matrix(1,0,0,1,0,40)` (translateY) after the `direction="up"` fix, and its left edge now matches the grid's left edge exactly (was 40px off before). `npx tsc --noEmit` clean. **DEPLOYED 2026-09-01** (`git pull && npm run build && pm2 restart eva-nepal`) — user confirmed successful deploy, not yet re-verified on a live device against the original DevTools repro.

**Files changed**: `app/globals.css`, `components/sections/ExecutiveCommittee.tsx`, `components/sections/StatsSection.tsx`, `components/sections/WhyJoin.tsx`, `components/sections/Events.tsx`, `components/sections/About.tsx`, `components/sections/MembershipForm.tsx`.

**Follow-up audit** (same day, in response to "can there be any other issues like those?"): grepped the whole codebase for the same 3 patterns rather than assuming the homepage sweep was exhaustive.
- Decorative glow-blob pattern (`blur-3xl`/`blur-[Npx]` + large fixed width): confirmed no other instances exist beyond the 4 already fixed.
- `AnimatedSection direction="left"/"right"`: confirmed no other instances exist beyond the 2 already fixed (About, MembershipForm).
- Unguarded `grid lg:grid-cols-N` + `lg:col-span-M` layout-split pattern: found **3 more instances** the homepage sweep never exercised because the sections had zero seeded data (News) or live on separate routes entirely (member-detail page, event-detail page) — `components/sections/News.tsx` (Editorial grid, `lg:col-span-2` + sidebar), `app/members/[slug]/page.tsx` (Profile card + Details), `app/events/[slug]/page.tsx` (Description/gallery + sidebar — this is the page holding `ImageLightboxGallery`, i.e. the "photo slider" the user's original report specifically named). Added `min-w-0` to all of them. Also fixed one instance in the admin panel (`app/(admin)/admin/members/[id]/page.tsx`) found by the same grep sweep, even though the admin surface isn't part of the public design mandate — the fix is zero-risk so there was no reason to leave it.
- **Honesty check on the follow-up fixes**: seeded temporary test data for News and for an event with `promoImages` and tried reverting each new `min-w-0` to see if overflow actually reproduced. It did **not** reproduce for News or the event-detail page with the test content used — both apparently already handle long content safely via their own truncation/overflow-x-auto (the gallery is a legitimately self-contained horizontal scroller). These 3 additions are therefore **defensive/preventive parity fixes** matching the confirmed-broken pattern, not independently confirmed live bugs — flagged as such rather than claimed as further "found and fixed" bugs, since only the original 3 root causes (blobs, Events.tsx grid, About/MembershipForm transform) were actually reproduced as real breakage.
- Checked `/portal`, `/portal/dues`, `/portal/meetings`, `/portal/events`, `/portal/profile`, `/members` (list): all clean, 0 overflow, full-page sweeps.
- Noted in passing, NOT fixed (out of scope for this task): `app/members/[slug]/page.tsx`'s page `<title>` renders literally `"null"` in it (`Sanat Mool – Member | null | Namo Udyam`) — a separate, real, unrelated bug in the metadata template, worth a follow-up.
